(function(){
   ...
   // gets a list of jquery objects to be used to access the time entry's data on the DOM
    ...
    $('#EntryList').on("keyup", 'input.entry-data[type="text"]', handleEdits)
    $('#EntryList').on('change','input.entry-data[type="date"], input.entry-data[type="time"]' ,handleEdits)
        //--- Listen for cancel, save and resume button clicks
    $('#EntryList').on('click','.cancel',function(e){
